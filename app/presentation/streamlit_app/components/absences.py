...
def absences_dashboard():
    ...
    with col_next:
        if st.button("Next week ➡️", key="abs_next_week"):
            ...
            st.rerun()
    with col_date:
        selected_date = st.date_input("📅 Select week (Monday only)", value=week_start, key="abs_date_input")
    ...
    if user["role"] != "manager":
        ...
        if st.button("Submit Absence Request", disabled=not days or not reason):
            payload = {
            ...
            try:
                res = requests.post(f"{BACKEND_URL}/absences/", json=payload, headers=get_auth_header())
                if res.status_code == 200:
                    st.success("✅ Absence request submitted!")
                    st.rerun()
                else:
                    st.error(f"❌ {res.json().get('detail', 'Failed to submit absence')}")
    ...
    for a in absences:
        ...
        st.markdown(f'<span style="background-color:{status_color}; color:black; padding:4px 12px; border-radius:8px; font-weight:bold;">{status.capitalize()}</span>', unsafe_allow_html=True)
    ...
    if user["role"] == "manager":
        ...
        mgr_resp = requests.get(f"{BACKEND_URL}/absences/project/{project_id}/week", params={"week_start": str(week_start)}, headers=get_auth_header())
        ...
        for a in mgr_absences:
            ...
            user_display = user_info["name"] + f" ({user_info['email']})" if user_info else a["user_id"]
            ...
            if status_desc:
                st.info(f"Status note: {status_desc}")
            if status == "pending":
                approve_desc = st.text_input(f"Approval description (optional) for {user_info['name'] if user_info else a['user_id']}", value="All ok", key=f"approve_desc_{a['absence_id']}")
                col1, col2 = st.columns(2)
                with col1:
                    if st.button(f"✅ Approve", key=f"approve_{a['absence_id']}"):
                        approve_url = f"{BACKEND_URL}/absences/approve/{a['absence_id']}"
                        resp = requests.post(approve_url, json={"description": approve_desc}, headers=get_auth_header())
                        if resp.status_code == 200:
                            st.success("✅ Absence approved!")
                            st.rerun()
                        else:
                            st.error(f"❌ {resp.json().get('detail', 'Failed to approve absence')}")
                with col2:
                    if st.button(f"❌ Decline", key=f"decline_{a['absence_id']}"):
                        st.session_state[f"show_decline_{a['absence_id']}"] = True
                    if st.session_state.get(f"show_decline_{a['absence_id']}", False):
                        decline_reason = st.text_area(f"Decline reason for {user_info['name'] if user_info else a['user_id']}", key=f"decline_reason_{a['absence_id']}")
                        submit_col, cancel_col = st.columns(2)
                        with submit_col:
                            if st.button(f"Submit Decline", key=f"submit_decline_{a['absence_id']}"):
                                if not decline_reason.strip():
                                    st.warning("Please provide a reason for declining.")
                                else:
                                    decline_url = f"{BACKEND_URL}/absences/decline/{a['absence_id']}"
                                    resp = requests.post(decline_url, json={"description": decline_reason}, headers=get_auth_header())
                                    if resp.status_code == 200:
                                        st.success("❌ Absence declined!")
                                        st.session_state[f"show_decline_{a['absence_id']}"] = False
                                        st.rerun()
                                    else:
                                        st.error(f"❌ {resp.json().get('detail', 'Failed to decline absence')}")
                        with cancel_col:
                            if st.button(f"Cancel", key=f"cancel_decline_{a['absence_id']}"):
                                st.session_state[f"show_decline_{a['absence_id']}"] = False
            st.markdown("---") 