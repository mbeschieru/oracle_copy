...
def fetch_attendance(meeting_id: UUID):
    resp = requests.get(
        f"{BACKEND_URL}/attendance/meeting/{meeting_id}",
...
def respond(meeting_id: UUID, status: str):
    ...
    if mine is None:
        ...
        att_id = next(
            a["meeting_attendance_id"]
            for a in fetch_attendance(meeting_id)
            if a["user_id"] == st.session_state.user["user_id"]
        )
        params = {"status": status}
        requests.patch(
            f"{BACKEND_URL}/attendance/{att_id}",
            headers=get_auth_header(),
            params=params,
        ).raise_for_status()
...
def calendar_dashboard():
    ...
    week_start_date = (today - timedelta(days=today.weekday())) + timedelta(
        weeks=week_offset
    )
    ...
    # Display as a table
    st.markdown(
        "<style>th, td {padding: 8px 12px;}</style>", unsafe_allow_html=True
    )
    st.write("### Week View")
    ...
    if selected_meeting_id:
        selected_meeting = next(
            (
                m
                for m in week_meetings
                if m["meeting_id"] == selected_meeting_id
            ),
            None,
        )
        if selected_meeting:
            ...
            with col1:
                if st.button(
                    "✅ Accept",
                    key=f"accept_{selected_meeting_id}",
            ...
            with col2:
                if st.button(
                    "❌ Decline",
                    key=f"decline_{selected_meeting_id}",
            ...
            try:
                ...
                accepted = [
                    resp for resp in responses if resp["status"] == "accepted"
                ]
            except Exception:
                accepted = []
            st.markdown("---")
