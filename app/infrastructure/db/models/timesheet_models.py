...
class TimeEntryModel(Base):
    ...
    timesheet_id = Column(
        CHAR(36), ForeignKey("timesheets.timesheet_id"), nullable=False
    )
    day = Column(Date, nullable=False)
    hours = Column(Float, nullable=False)
    project_id = Column(
        CHAR(36), ForeignKey("projects.project_id"), nullable=False
    )
...
class TimesheetModel(Base):
    ...
    entries = relationship(
        "TimeEntryModel", backref="timesheet", cascade="all, delete-orphan"
    )
