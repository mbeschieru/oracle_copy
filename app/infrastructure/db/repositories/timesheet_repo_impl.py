...
class TimesheetRepository(TimesheetRepositoryInterface):
    ...
    def get_by_user(self, user_id):
        with SessionLocal() as session:
            return session.query(TimesheetModel).options(
                joinedload(TimesheetModel.entries)
            ).filter(TimesheetModel.user_id == user_id).all()
    ...
    def get_by_id(self, timesheet_id):
        with SessionLocal() as session:
            return session.query(TimesheetModel).options(
                joinedload(TimesheetModel.entries)
            ).filter(TimesheetModel.timesheet_id == timesheet_id).first()

    def save(self, timesheet):
        with SessionLocal() as session:
            existing = session.query(TimesheetModel).filter(TimesheetModel.timesheet_id == str(timesheet.timesheet_id)).first()
            if existing:
                # Update fields
    ...
    def approve(self, timesheet_id: UUID) -> None:
        with SessionLocal() as session:
            timesheet = session.query(TimesheetModel).filter(TimesheetModel.timesheet_id == str(timesheet_id)).first()
            if not timesheet:
