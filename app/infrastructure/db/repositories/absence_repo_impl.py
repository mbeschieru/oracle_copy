...
class AbsenceRepository(AbsenceRepositoryInterface):
    ...
    def get_by_user(self, user_id: UUID):
        with SessionLocal() as session:
            absences = (
                session.query(AbsenceModel)
                .filter(AbsenceModel.user_id == str(user_id))
                .all()
            )
    ...
    def get_by_id(self, absence_id: UUID):
        with SessionLocal() as session:
            a = (
                session.query(AbsenceModel)
                .filter(AbsenceModel.absence_id == str(absence_id))
                .first()
            )
    ...
    def save(self, absence: Absence):
        with SessionLocal() as session:
            existing = (
                session.query(AbsenceModel)
                .filter(AbsenceModel.absence_id == str(absence.absence_id))
                .first()
            )
    ...
    def get_by_project_and_week(self, project_id: UUID, week_start: date):
        ...
        with SessionLocal() as session:
            absences = (
                session.query(AbsenceModel)
                .join(UserModel, AbsenceModel.user_id == UserModel.user_id)
                .filter(
