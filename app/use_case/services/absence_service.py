...
class AbsenceService:
    def __init__(self, absence_repo: AbsenceRepositoryInterface, user_repo: UserRepositoryInterface):
    ...
    def create_absence(self, dto: AbsenceCreateDTO, user_id: UUID):
        ...
        if dto.week_start.weekday() != 0:
            raise HTTPException(status_code=400, detail="Absences can only be created for weeks starting on Monday.")
        # Check for existing absence for this week
        existing = self.repo.get_by_week(user_id, dto.week_start)
        if existing:
            raise HTTPException(status_code=409, detail="Absence already exists for this week.")
        absence = Absence(
    ...
    def get_absence_by_week(self, user_id: UUID, week_start: date):
        ...
        if not absence:
            raise HTTPException(status_code=404, detail="Absence not found for this week.")
        return AbsenceReadDTO.model_validate(absence)

    def approve_absence(self, absence_id: UUID, manager_id: UUID, description: str = "All ok"):
        ...
        if manager.project_id != user.project_id:
            raise permission_denied("You are not authorized to approve this absence.")
    ...
    def decline_absence(self, absence_id: UUID, manager_id: UUID, reason: str):
        ...
        user = self.user_repo.get_by_id(absence.user_id)
        if manager.project_id != user.project_id:
            raise permission_denied("You are not authorized to decline this absence.")
