...
class TimesheetService:
    ...
    def __init__(
        self,
        timesheet_repo: TimesheetRepositoryInterface,
    ...
    def get_timesheets_for_user(
        self, requester_id: UUID, target_user_id: UUID
    ):
        assert_user_can_access_user(
            requester_id, target_user_id, self.user_repo
        )
    ...
    def get_timesheet_by_week(
        self, requester_id: UUID, target_user_id: UUID, week_start: date
    ):
        assert_user_can_access_user(
            requester_id, target_user_id, self.user_repo
        )
        ...
        if not ts:
            raise user_timesheet_not_found(
                str(target_user_id), str(week_start)
            )
    ...
    def submit_timesheet(self, dto: TimesheetCreateDTO, user_id: UUID):
        ...
        if user.role == UserRole.MANAGER:
            raise permission_denied(
                "Managers are not allowed to submit timesheets."
            )
        # Enforce week_start is Monday
        if dto.week_start.weekday() != 0:
            raise HTTPException(
                status_code=400,
                detail="Timesheets can only be created"
    ...
    def approve_timesheet(
        self, timesheet_id: UUID, manager: User, description: str = "All ok"
    ):
        ...
        if (
            ...
            or manager.project_id != employee.project_id
        ):
            raise permission_denied(
    ...
    def decline_timesheet(
        self, timesheet_id: UUID, manager: User, reason: str
    ):
        timesheet = self.repo.get_by_id(timesheet_id)
        if not timesheet:
            raise timesheet_not_found(str(timesheet_id))
        ...
        if (
            ...
            raise permission_denied(
                "You are not authorized to decline this timesheet"
            )
