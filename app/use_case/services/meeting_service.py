...
class MeetingService:
    ...
    def get_meeting_attendance(
        ...
        attendance_data, total_count = (
            self.attendance_repo.get_attendance_for_meeting(
    ...
    def create_meeting(self, data: MeetingCreateDTO, user_id: UUID):
        ...
        from app.infrastructure.db.repositories.user_repo_impl import (
            UserRepository,
        )
        ...
        print(
            f"[DEBUG] Creating meeting as user: {user.email}, "
            f"role: {user.role}"
        )
