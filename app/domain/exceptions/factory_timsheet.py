...
def user_timesheet_not_found(user_id: str, week_start: str = None) -> DomainException:
    details = (
        f"User with id {user_id} does not have a timesheet submitted for the week starting {week_start}"
        if week_start else
        f"User with id {user_id} does not have any timesheets submitted"
    )
...
def duplicate_timesheet(user_id: str, week_start: str) -> DomainException:
    return DomainException(
        ...
        details=f"User {user_id} has already submitted a timesheet for week starting {week_start}",
        http_code=409
    )
